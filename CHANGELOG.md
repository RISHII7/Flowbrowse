...
> **Highlights** 📄 A spec for two loose ends: a Run/Stop toggle on the workflow Run button, and wiring the dashboard empty state's "New workflow" button. Spec only, no implementation.

### ✨ Added

- **`specs/polish.md`** — a two-part plan: turn the Run button into a Run/Stop toggle that cancels the in-flight run via the existing cancel action, using the runs provider's live-run tracking (at most one run live at a time); and wire the dashboard empty state's "New workflow" button to a direct call to the create-workflow action, giving the new workflow a generated name the same way the sidebar's button does — no optimistic update.
...
> **Highlights** 📄 `specs/billing.md`'s Prompt 1 now says explicitly not to write any app code yet — enabling billing and creating the plan is config only.

### ♻️ Changed

- **`specs/billing.md`** — Prompt 1 adds a line scoping it to configuration only: no app code, no pricing/billing screen — that's Prompt 2.

---

## [0.30.3] — 2026-07-31

> **Highlights** 📄 Split `specs/billing.md`'s setup prompt into two, one concrete deliverable each, matching the rest of the spec's shape.

### ♻️ Changed

- **`specs/billing.md`** — Prompt 1 (enable billing and create the plan) and the new Prompt 2 (build the pricing page and pro-gate hook) used to be one prompt. The dev-instance payment-gateway note now sits directly under the plan setup it applies to. The gate prompts renumber to 3 and 4.

---

## [0.30.2] — 2026-07-31

> **Highlights** 📄 Prompt 1 and Prompt 3 of `specs/billing.md` now explicitly call out the `clerk-billing` skill, instead of leaving it to the spec's intro note.

### ♻️ Changed

- **`specs/billing.md`** — Prompt 1 (initial org billing/plan setup) and Prompt 3 (the server-side plan check when gating workflow creation) now say to use the `clerk-billing` skill directly.
